    ...
    <div class="py-6">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.donors.update', $donor->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                            ...
                            <div>
                                ...
                                @if ($donor->profile_photo)
                                    <div class="mt-4">
                                        <div class="text-sm font-medium text-gray-700 mb-1">{{ __('Current Photo') }}</div>
                                    ...
                                    <p class="mt-1 text-sm text-gray-500">
                                        {{ __('Leave empty to keep current photo, or upload a new one to replace it.') }}
                                    </p>
                                @else
                                    <p class="mt-1 text-sm text-gray-500">{{ __('No photo currently set.') }}</p>
                                @endif
                            </div>
                                ...
                                <select id="blood_group_id" name="blood_group_id"
                                    ...
                                    @foreach ($bloodGroups as $bloodGroup)
                                        <option value="{{ $bloodGroup->id }}"
                                            {{ old('blood_group_id', $donor->blood_group_id) == $bloodGroup->id ? 'selected' : '' }}>
                                            {{ $bloodGroup->name }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <div class="mt-1 flex items-center space-x-4">
                                    <div class="flex items-center">
                                        <input id="gender_male" type="radio" name="gender" value="male"
                                            class="rounded border-gray-300 text-indigo-600 shadow-sm {{ old('gender', $donor->gender) == 'male' ? 'checked' : '' }}">
                                        <label for="gender_male"
                                            class="ml-2 text-sm text-gray-700">{{ __('Male') }}</label>
                                    </div>
                                    <div class="flex items-center">
                                        <input id="gender_female" type="radio" name="gender" value="female"
                                            class="rounded border-gray-300 text-indigo-600 shadow-sm ml-6 {{ old('gender', $donor->gender) == 'female' ? 'checked' : '' }}">
                                        <label for="gender_female"
                                            class="ml-2 text-sm text-gray-700">{{ __('Female') }}</label>
                                    </div>
                                    <div class="flex items-center">
                                        <input id="gender_other" type="radio" name="gender" value="other"
                                            class="rounded border-gray-300 text-indigo-600 shadow-sm ml-6 {{ old('gender', $donor->gender) == 'other' ? 'checked' : '' }}">
                                        <label for="gender_other"
                                            class="ml-2 text-sm text-gray-700">{{ __('Other') }}</label>
                                    </div>
                                </div>
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <textarea id="address" name="address" rows="3"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary focus:ring-opacity-50 sm:text-sm"
                                    required>{{ old('address', $donor->address) }}</textarea>
                                ...
                                <div class="mt-1 flex items-center space-x-4">
                                    <div class="flex items-center">
                                        <input id="available" type="radio" name="availability_status" value="available"
                                            class="rounded border-gray-300 text-indigo-600 shadow-sm {{ old('availability_status', $donor->availability_status) == 'available' ? 'checked' : '' }}">
                                        <label for="available"
                                            class="ml-2 text-sm text-gray-700">{{ __('Available') }}</label>
                                    </div>
                                    <div class="flex items-center">
                                        <input id="not_available" type="radio" name="availability_status"
                                            value="not_available"
                                            class="rounded border-gray-300 text-indigo-600 shadow-sm ml-6 {{ old('availability_status', $donor->availability_status) == 'not_available' ? 'checked' : '' }}">
                                        <label for="not_available"
                                            class="ml-2 text-sm text-gray-700">{{ __('Not Available') }}</label>
                                    </div>
                                </div>
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <textarea id="notes" name="notes" rows="4"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary focus:ring-opacity-50 sm:text-sm">{{ old('notes', $donor->notes) }}</textarea>
